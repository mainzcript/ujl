...
export interface Config {
  ...
  collectionsSelect: {
    ...
    access_tokens: AccessTokensSelect<false> | AccessTokensSelect<true>;
...
/**
 * Short-lived tokens for API access. Issued via POST /api/access-tokens/issue.
 *
 * This interface was referenced by `Config`'s JSON-Schema
 * via the `definition` "access_tokens".
 */
export interface AccessToken {
  id: number;
  /**
   * The secret token string (Bearer).
   */
  token: string;
  /**
   * User (API key holder) who requested this token.
   */
  user: number | User;
  /**
  ...
  issuedAt: string;
  /**
   * When the token expires (typically issuedAt + 15 min).
   */
  expiresAt: string;
  /**
   * Client IP when token was requested (audit).
   */
  ipAddress?: string | null;
  /**
   * Client User-Agent when token was requested (audit).
   */
...
/**
 * This interface was referenced by `Config`'s JSON-Schema
 * via the `definition` "access_tokens_select".
 */
