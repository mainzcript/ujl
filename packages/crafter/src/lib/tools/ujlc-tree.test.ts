...
import {
	...
	canAcceptDrop,
	canNodeAcceptPaste
} from '$lib/tools/ujlc-tree.ts';
import { createMockNode, createMockTree, createMockMultiSlotTree } from '$lib/../tests/mockData.js';
...
describe('ujlc-tree-utils', () => {
	...
	describe('updateNodeInTree', () => {
		...
		it('should be immutable', () => {
			...
			// Original unchanged
			expect(original?.fields.content).toBe('Leaf 1');
			// Result updated
			const updated = findNodeById(result, 'leaf-1');
			expect(updated?.fields.content).toBe('Updated');
			expect(updated).not.toBe(original);
