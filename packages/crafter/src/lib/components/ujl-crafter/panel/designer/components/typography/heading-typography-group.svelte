...
<script lang="ts">
	...
	let {
		...
		onChange?: (updates: Partial<UJLTTypographyHeading>) => void;
	} = $props();
	...
	function handleUpdate(field: keyof UJLTTypographyHeading, value: unknown) {
		if (onChange) {
			onChange({ [field]: value } as Partial<UJLTTypographyHeading>);
		}
	}
</script>
