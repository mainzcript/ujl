...
### 6.1.1 Szenario 1: Bearbeitung einer `.ujlc.json`-Datei

```plantuml
@startuml
title Bearbeitung einer .ujlc.json-Datei: Hinzufügen eines Text-Moduls innerhalb eines Containers
...
CM -> Editor: Öffnet .ujlc.json-Datei
activate Editor
Editor -> Storage: Lädt .ujlc.json-Datei
activate Storage
Storage --> Editor: .ujlc.json-Datei-Daten
deactivate Storage
...
Editor -> Storage: Speichert aktualisierte .ujlc.json-Datei
activate Storage
...
1. Der **Redakteur** öffnet eine `.ujlc.json`-Datei im **UJL Crafter (Editor)**. Der Editor lädt die Datei aus dem **Speicher**.
...
5. Abschließend speichert der **Editor** die aktualisierte `.ujlc.json`-Datei im **Speicher**.
...
### 6.1.2 Szenario 2: Rendern und Anzeigen einer `.ujlc.json`-Datei in einer Web-Ansicht
...
Renderer -> Speicher: Lädt .ujlc.json-Datei
activate Renderer
activate Speicher
Speicher --> Renderer: .ujlc.json-Datei-Daten
deactivate Speicher
...
Das zweite Beispiel veranschaulicht den Ablauf, wenn eine `.ujlc.json`-Datei für die Anzeige in einer Web-Ansicht gerendert wird. Dieser Ablauf zeigt, wie der **Renderer** die Datei verarbeitet und als ContentFrame (HTML/CSS/JS) bereitstellt:

1. Der **UJL Renderer** lädt die `.ujlc.json`-Datei mit Layout- und Inhaltsdaten aus dem **Speicher**.
